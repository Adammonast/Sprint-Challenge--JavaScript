...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++){
  const contact = `${graduates[i].first_name} ${graduates[i].email}`
...
*/
let display = []
  const displayName = zooAnimals.forEach((value)=>{
    return display.push(`${value.animal_name}, ${value.scientific_name}`)
  })
...
*/
const lowerCase = []
  const lower = zooAnimals.map((value) => { const name = value.animal_name
...
let lowPopulation = []
let low = []
low = zooAnimals.filter((less)=>{
  return less.population < 5
})
lowPopulation.push(low);
...
*/
let pop = 0
pop = zooAnimals.reduce((a, b)=>{
return a + b.population 
}, 0)
const populationTotal = pop;
